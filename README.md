...
[Quick Start](#quick-start) • [The Three Pillars](#the-three-pillars) • [Dashboard](#dashboard) • [Connect an Agent](#connect-an-agent) • [MCP Tools](#mcp-tools) • [Configuration](#configuration) • [Auth](#authentication) • [Your Data](#where-your-data-lives) • [Upgrades](#schema-versions-and-upgrades)
...
**Platforms.** Windows, macOS and Linux, on x64 and arm64. Everything builds AnyCPU — no fixed
...
Only the matching native is pulled in — each is tens of megabytes and a sidecar ships for one
platform. On a runtime identifier ClearScript does not publish a V8 for, the build still succeeds
and says so: TypeScript code intelligence reports itself unavailable and everything else works.
...
> Building an AI companion on top of this? **[companion-api.md](companion-api.md)** covers the whole
> surface end to end — the scoping model, the extraction pass that decides what to remember after
> each turn, real-time recall over MCP, the conflict loop, every payload, and the traps.
...
| `list_conflicts` | Contradictions awaiting a decision |
| `resolve_conflict` | Settle one once the user has clarified — the loser becomes history, never deleted |
...
| **Slots & conflicts** | `GET /api/memory/{slots,slot,conflicts}`, `POST /api/memory/conflicts/{id}/resolve` |
...
**Memory scoring.** Retrieval runs five independent channels — vector, lexical (BM25F), slot, recency
and link — and combines them with **Reciprocal Rank Fusion**, then diversifies with MMR. RRF consumes
each channel's *ranking* rather than its score, because a cosine and a BM25 value are not on the same
scale and summing them weighted lets whichever channel happens to produce larger numbers dominate.
Channel weights live under `Retrieval` in `appsettings.json`. Strength follows
...
can read and write your memories with an unauthenticated `GET`. That is fine on a machine you
control and nowhere else — the startup banner says so in as many words.
...
| **Open** | The dashboard's static assets — a browser cannot attach a header to its own page load. The data they request is protected. |
...
There is deliberately **no command-line flag** — process arguments are readable by any other process
on Windows, Linux and macOS alike, so a secret passed that way is not a secret. For the same reason
the shipped `appsettings.json` never carries a key, and a build-failing test enforces it: a key
committed to the repository is identical on every install, so recovering it once unlocks everybody.
...
> This is a single shared secret, not user accounts — the right shape for one local process serving
> one person. It does not encrypt anything; use loopback binding as well.
...
read-only in a signed macOS bundle, needs elevation under `%ProgramFiles%`, and — the one that
destroys data — **is replaced wholesale on every auto-update**. A database kept there is deleted the
...
`portable.txt` file beside the executable keeps everything in `./Data` instead — useful for a
checkout or a USB stick.
...
- **Newer than this build** → **the server refuses to start.** Old code against a new schema is not
  a degraded experience, it is silent destruction: it reads fields it does not understand, drops
  them on the next write, and leaves a file no version can make sense of. For an app that
  auto-updates — where a user can roll back, or a stale sidecar can be launched beside a current
  one — that is a routine accident, so it exits non-zero with the reason on the console rather than
...
**511 tests**, xUnit v3. Alongside the usual unit coverage there are four suites that exist to fail
